...
export default function Pdf() {
  ...
  const handleQuerySubmit = (e: any) => {
    ...
    setLoading(true);
    client
      .post("/explain", { query: explainQuery })
      .then((res) => {
      ...
      <div className='flex gradient h-screen'>
        ...
        <div className='flex flex-col w-[50vw] h-screen'>
          <RoughNotation
            ...
            show={true}
          >
            <h1 className='text-2xl text-center border-b-2 border-green-200 font-bold m-4 p-2 pb-6'>
              {/* {slug} */}
              Navigating through the PDF. One question at a time.
            </h1>
          </RoughNotation>
          <Chatbot />
          ...
          >
            ...
            >
              <textarea
                rows={6}
                className='rounded-md focus:outline-none border-2 py-4 px-8 w-[80%]'
                value={explainQuery}
            ...
            {response2 && (
              <div className='m-4 p-4 w-[40vw]'>{response2?.answer}</div>
            )}
            {loading && <div className='m-4 p-4'>Loading ...</div>}
          </motion.div> */}
        </div>
      </div>
    </Layout>
